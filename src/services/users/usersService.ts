...
export const getUserProfileById = async ({
  ...
  try {
    const response = await networkClient.get(`/v1/users/${encodeURIComponent(userId)}`, {headers});
    ...
    throw new Error(getServiceErrorMessage(error, 'Kullanıcı detayı alınamadı.'));
  }
};
